...
@patch("session.finish_active_task")
@patch("session.emit")
@patch("session.BrowserAgent")
def test_run_task_finishes_active_task_when_agent_setup_fails(
    mock_browser_agent,
    mock_emit,
    mock_finish_active_task,
):
    browser = MagicMock()
    ...
    mock_finish_active_task.assert_called_once()
    emitted_types = [call.args[0]["type"] for call in mock_emit.call_args_list]
    assert "step_error" in emitted_types
    assert "task_failed" in emitted_types
    assert "task_complete" not in emitted_types
